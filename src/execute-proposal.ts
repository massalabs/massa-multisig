...
function parseCliArgs(): { multisigAddress: string; proposalId: bigint } {
  ...
  if (!multisigAddress || !proposalIdRaw) {
    throw new Error(
      'Usage: npm run execute:proposal -- <multisig-address> <proposal-id>',
    );
  }
...
const executeArgs = new Args().addU64(proposalId).serialize();

console.log(`Executing proposal ${proposalId} on ${multisigAddress}`);

const operation = await multisig.call('execute', executeArgs);
...
if (finalStatus !== OperationStatus.Success) {
  throw new Error(
    `Proposal execution failed with status ${OperationStatus[finalStatus]}`,
  );
}
